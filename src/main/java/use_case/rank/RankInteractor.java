...
public class RankInteractor implements RankInputBoundary {
    ...
    public RankInteractor(RankUserDataAccessInterface dao,
                          RankOutputBoundary presenter,
    ...
    public void execute(RankInputData in) {
        final List<Map.Entry<String, Integer>> users = scoreSortService.getSortedRank(dao.getAllUsers());

        final LeaderboardStat leaderboardStat = leaderboardSelector.getSortedRank(users, in.getUsername(), limit);
